...
export const GET: APIRoute = async (context) => {
  ...
  if (!emoticonsStore) {
    // Local dev mock system
    if (fileName) {
      // Serve a beautifully designed SVG emoji based on emotion filename
  ...
  try {
    ...
    const emoticonFiles = list.objects
      .filter((obj: any) => obj.key.toLowerCase().endsWith('.png') || obj.key.toLowerCase().endsWith('.webp') || obj.key.toLowerCase().endsWith('.gif'))
      .map((obj: any) => ({
        key: obj.key,
